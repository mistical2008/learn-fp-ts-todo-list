...
for (let i = 1; i < 100; i++) {
  ...
  const suffix = faker.fake('{{name.suffix}}');
  raw_data.push({
    name: {
    ...
    email: {
      formatted: <EuiLink href="">{faker.fake('{{internet.email}}')}</EuiLink>,
      raw: email,
    },
    location: (
      <Fragment>
    ...
    account: faker.fake('{{finance.account}}'),
    amount: faker.fake('${{commerce.price}}'),
    phone: faker.fake('{{phone.phoneNumber}}'),
    version: faker.fake('{{system.semver}}'),
  });
}
